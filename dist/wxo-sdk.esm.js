...
class ChatWindow {
  ...
  _renderWelcomeScreen() {
    var prompts = Array.isArray(this.agent.quickStartPrompts) ? this.agent.quickStartPrompts : [];
    var welcome = this.agent.welcomeMessage || "".concat(this.agent.name, "\u3078\u3088\u3046\u3053\u305D");
    var subtitle = this.agent.welcomeSubtitle || '';
    this.welcomeEl = document.createElement('div');
    this.welcomeEl.className = 'wxo-welcome';
    this.welcomeEl.innerHTML = "\n      <div class=\"wxo-welcome__icon\">".concat(this.agent.icon || '💬', "</div>\n      <div class=\"wxo-welcome__title\">").concat(this._escapeHtml(welcome), "</div>\n      ").concat(subtitle ? "<div class=\"wxo-welcome__subtitle\">".concat(this._escapeHtml(subtitle), "</div>") : '', "\n      ").concat(prompts.length > 0 ? "<div class=\"wxo-welcome__prompts\">".concat(prompts.map(p => "<button class=\"wxo-welcome__prompt\">".concat(this._escapeHtml(p), "</button>")).join(''), "</div>") : '', "\n    ");
